...
def encoder(input):
  ...
  l3 = tf.nn.sigmoid(tf.add(tf.matmul(l2, weights['encoder_3']), biases['encoder_3']))
  l4 = tf.nn.sigmoid(tf.add(tf.matmul(l3, weights['encoder_4']), biases['encoder_4']))
  return l4


def decoder(hidden_layer):
  l4 = tf.nn.sigmoid(tf.add(tf.matmul(hidden_layer, weights['decoder_4']), biases['decoder_4']))
  l3 = tf.nn.sigmoid(tf.add(tf.matmul(l4, weights['decoder_3']), biases['decoder_3']))
  l2 = tf.nn.sigmoid(tf.add(tf.matmul(l3, weights['decoder_2']), biases['decoder_2']))
...
with tf.Session() as sess:
  ...
  validate = sess.run(y_pred, feed_dict={X: mnist.test.images})
  # print(validate)

  f, a = plt.subplots(2, 10, figsize=(10, 2))
  for i in range(10):
    a[0][i].imshow(np.reshape(mnist.test.images[i], (28, 28)))
    a[1][i].imshow(np.reshape(validate[i], (28, 28)))
  plt.show()