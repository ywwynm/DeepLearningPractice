...
def aein_net(input):
  net = conv_2d(input, filter_size=11, in_channel=3, out_channel=64, strides=4, padding='SAME')
  net = max_pool(net, ksize=3, strides=2, padding='VALID')

  net = conv_2d(net, filter_size=4, in_channel=64, out_channel=128, padding='SAME')
  net = max_pool(net, ksize=3, strides=2, padding='VALID')

  net = conv_2d(net, filter_size=4, in_channel=128, out_channel=256, padding='SAME')
  net = conv_2d(net, filter_size=4, in_channel=256, out_channel=384, padding='SAME')
  net = max_pool(net, ksize=3, strides=2, padding='VALID')

  net = fully_connected(net, n_units=1024, activation="relu", keep_prob=0.5)
  net = fully_connected(net, n_units=1024, activation="relu", keep_prob=0.5)
