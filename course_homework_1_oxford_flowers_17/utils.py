...
def save_result(net_name, epochs_arr, losses, epochs_10_arr, accuracies):
  ...
  plt.xlabel('epoch')
  plt.ylabel('loss')
  plt.savefig("result/" + net_name + "_loss_" + post_fix + ".png")

  plt.clf()  # clear existing figure content
  plt.plot(epochs_10_arr, accuracies)
  plt.xlabel('epoch')
  plt.ylabel('accuracy')
  plt.savefig("result/" + net_name + "_acc_" + post_fix + ".png")
